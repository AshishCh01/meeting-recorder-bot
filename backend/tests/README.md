...
The chat stream-error and AI-usage tests need only Postgres too. Full suite: 412
with Redis, 349 passed / 63 skipped without.
...
suite reports `349 passed, 63 skipped` and exits **0**. Those 63 are B1's
...
— run without Redis and you get the 349 tests that do not need one, plus a note
about what you missed, instead of a red suite.

It refuses *any* skip, not just Redis ones. There is no legitimately
conditional test here today (with both services: `412 passed`, zero skipped),
